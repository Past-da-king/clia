# ai_agent/agent.py (with Conversational Memory)
import asyncio
import os
import sys
import textwrap
...
import subprocess # Added for subprocess management
...
async def main(server_process):
    ...
    print(f" Looking for tool server: {MCP_SERVER_SCRIPT}")

    async with ClientSession(server_process.stdout, server_process.stdin) as mcp_session:
...
Always think step-by-step. Use the conversational history to inform your plan. Announce your plan, execute a single tool, analyze the result, and then announce your next plan. Your goal is to be a transparent and reliable engineer.
        ...
        while True:
            ...
            while turn_count < MAX_TOOL_TURNS:
                ...
                print(f"  AI wants to run: {tool_name}({', '.join(f'{k}={v!r}' for k, v in tool_args.items())})")
            ...
            if turn_count >= MAX_TOOL_TURNS:
                print("  Warning: Maximum tool turns reached. Task may be incomplete.")


if __name__ == "__main__":
    print("Starting MCP Tool Server...ayanda")
    server_process = None
    try:
        server_process = subprocess.Popen(
            [sys.executable, "-m", MCP_SERVER_SCRIPT],
        ...
        asyncio.run(main(server_process))
